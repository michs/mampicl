...
static CU_ErrorCode basic_initialize(void)
{
  ...
  if (CU_BRM_SILENT != f_run_mode && myrank == 0)
    fprintf(stdout, "\n\n     %s" CU_VERSION
                      "\n     %s\n\n",
                                                ...
                                                const CU_pFailureRecord pFailureList)
{
  ...
  int numFailures[numprocs], nFailure = 0;
  CU_pFailureRecord tmpFail;

  for (tmpFail = pFailure; tmpFail != NULL; tmpFail = tmpFail->pNext)
    nFailure += 1;

  if ( myrank == 0)
  {
    MPI_Gather(&nFailure, 1, MPI_INT, numFailures, 1, MPI_INT, 0, MPI_COMM_WORLD);
  }
  else
    MPI_Gather(&nFailure, 1, MPI_INT, NULL, 0, MPI_INT, 0, MPI_COMM_WORLD);
  nFailure = 0;
  for (i = 0; i < numprocs; i++)
    nFailure += numFailures[i];
  ...
  if (CU_BRM_SILENT != f_run_mode && myrank != 0)
  {
    ...
    for (i = 1 ; (NULL != pFailure) ; pFailure = pFailure->pNext, i++) {
      sprintf(sendbuff, "\n    [%d] %d. %s:%u  - %s", myrank, i,
          ...
          pFailure->uiLineNumber,
          (NULL != pFailure->strCondition) ? pFailure->strCondition : "");
      MPI_Send(sendbuff, strlen(sendbuff)+1, MPI_CHAR, 0, 0, MPI_COMM_WORLD);
    }
  }

  if ( myrank != 0 )  /* From here only output on rank 0 */
    return;

  if (NULL == pFailure && nFailure == 0)
  {
    if (CU_BRM_VERBOSE == f_run_mode)
  ...
  else
  {
    ...
    if (CU_BRM_SILENT != f_run_mode)
    {
      ...
      for (i = 1; i < numprocs; i++)
        if ( numFailures[i] > 0 )
        {
          for (j = 0; j < numFailures[i]; j++)
          {
            MPI_Status status;
            char recvbuff[512];

            MPI_Recv(recvbuff, 512, MPI_CHAR, i, 0, MPI_COMM_WORLD, &status);
